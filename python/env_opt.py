...
def optimize_shape(scene_config, mts_args, ref_image_paths,
    ...
    sdf_scene = mi.load_file(ref_scene_name, integrator='prb', 
                             resx=scene_config.resx, resy=scene_config.resy, **mts_args, parallel=config.use_parallel_loading)
    ...
    try:
        ...
        for i in pbar:
            ...
            if dr.grad_enabled(loss):
                loss_str += f' (loss (avg. x 1e4): {1e4*loss[0] :.4f})'
